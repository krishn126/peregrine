...
slope, intercept, r_value, _, _ = linregress(log_x_prime, log_y_prime)

# Generate best-fit line in log space
x_fit = np.logspace(np.log10(min(x_prime)), np.log10(max(x_prime)), 100)  # Log-spaced x values
...
plt.title('How Validation Loss Changes with Number of Hidden Features')
plt.legend()
