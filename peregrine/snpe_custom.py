    ...
    for round_id in range(1, int(conf["snpe"]["num_rounds"]) + 1):
        ...
        if (
            ...
            for i in range(15):
                ...
                plt.ylabel("Density")
                plt.title(f"Posterior Distribution for the parameter {order[i]} without noise reshuffling")
                plt.legend()
                plt.savefig(f"/data/kn405/Code/peregrine_snpe/peregrine/posterior_plots/posterior_for_{order[i]}_no_noise_re.png", dpi=300, bbox_inches='tight')        