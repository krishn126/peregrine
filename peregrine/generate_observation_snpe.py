...
if __name__ == "__main__":
    args = sys.argv[1:]
    print(
        f"{datetime.now().strftime('%a %d %b %H:%M:%S')} | [generate_observation_snpe.py] | Reading config file"
    )
    ...
    with open(
        ...
        pickle.dump(obs, f)
    print(
        f"{datetime.now().strftime('%a %d %b %H:%M:%S')} | [generate_observation_snpe.py] | Generated observation"
    )
    print(f"\nConfig: {args[0]}")
    print(
